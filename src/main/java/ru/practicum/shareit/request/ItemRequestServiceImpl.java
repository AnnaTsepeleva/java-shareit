...
public class ItemRequestServiceImpl implements ItemRequestService {
    ...
    public GetItemRequestDto createRequest(long userId, CreateItemRequestDto itemRequestDto) {
        User requester = userStorage.findById(userId).orElseThrow(() -> new NotFoundException("Пользователь не найден"));
    ...
    public List<GetItemRequestDto> getAllRequestsByUserId(long userId) {
        ...
        return itemRequestStorage.getAllByRequester(requester, SORT_BY_CREATED_DESC).stream().map(ItemRequestMapper::toGetItemRequestDtoFromItemRequest).collect(Collectors.toList());
    }
    ...
    public List<GetItemRequestDto> getAllRequests(long userId, int from, int size) {
        ...
        return itemRequestStorage.getAllByRequesterNot(requester, pageable).stream().map(ItemRequestMapper::toGetItemRequestDtoFromItemRequest).collect(Collectors.toList());
    }
    ...
    public GetItemRequestDto getRequestById(long userId, long requestId) {
        ...
        return ItemRequestMapper.toGetItemRequestDtoFromItemRequest(itemRequestStorage.findById(requestId).orElseThrow(() -> new NotFoundException("Запрос на вещь не найден")));
    }
}